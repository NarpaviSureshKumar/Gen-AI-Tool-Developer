...
async function main() {
  ...
  try {
    ...
    // Load testcases
    // Automatically pick the latest JSON file from src/data/
    ...
    if (files.length === 0) {
      throw new Error("No JSON files found in src/data/");
    }
    const latestFile = path.join(dataDir, files[0].name);
    console.log("Using latest JSON file:", latestFile);
    const testcases = JSON.parse(fs.readFileSync(latestFile, "utf-8"));
